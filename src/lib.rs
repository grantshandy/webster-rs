...
//! and then decompressed upon runtime access. The runtime container provides `O(log n)` access complexity and access time (anecdotally) faster than a [`std::collections::BTreeMap`].
...
pub fn dictionary<T: Into<String>>(word: T) -> Option<&'static str> {
    let key = word.into().to_string().to_uppercase();

    DICTIONARY
        .binary_search_by(|[w, _]| w.cmp(&key))
        .ok()
